...
# Persistent browser profile (keeps Jira + S2AP login sessions)
...
def handle_jira_login_required(page):
    ...
    print(f"========================================")
    print(f"The persistent browser profile requires login for Jira.")
...
def handle_s2ap_login_required(page):
    print(f"\n========================================")
    print(f"ACTION REQUIRED: S2AP LOGIN")
    print(f"========================================")
    print(f"The S2AP session expired or is logged out.")
    print(f"Please sign in (e.g. via Entra ID) in the opened browser window.")
    
    # Auto-resume once the URL leaves login/auth paths
    while "/realms/" in page.url.lower() or "openid-connect" in page.url.lower() or "login" in page.url.lower():
        time.sleep(2)

    print("\nLogin detected! Waiting for dashboard to finish loading...")
...
def open_s2ap_and_shortcut(page, s2ap_url):
    ...
    current_url = page.url.lower()
    if "/realms/" in current_url or "openid-connect" in current_url or "login" in current_url:
        handle_s2ap_login_required(page)
